...
\title{Gramática de  operaciones aritméticas}
...
Este proyecto desarrolla una aplicación web para generar automáticamente casos de prueba válidos, 
inválidos y extremos a partir de una gramática libre de contexto enfocada en expresiones aritméticas. 
La herramienta utiliza técnicas de derivación y mutación sintáctica para crear estos casos, permitiendo 
evaluar el comportamiento de software que procesa expresiones matemáticas. 
Implementada en Svelte con TypeScript, la aplicación incluye una interfaz intuitiva para cargar gramáticas, 
configurar parámetros y exportar resultados en formato JSON. 

\textbf{Palabras clave:} Gramática Libre de Contexto, Generación de Casos de Prueba, Expresiones Aritméticas.
...
En el contexto de la Teoría de la Computación, las gramáticas formales permiten definir lenguajes de manera precisa. Una Gramática Libre de Contexto (GLC) es un tipo de gramática donde las reglas de producción tienen la forma A → α, donde A es un símbolo no terminal y α es una cadena de símbolos terminales y no terminales. Estas gramáticas generan lenguajes que pueden ser reconocidos por autómatas de pila.

El proyecto aplica estos conceptos para generar casos de prueba sistemáticos para software que procesa expresiones aritméticas. Esto incluye operaciones como suma (+), resta (-), multiplicación (*), división (/), y módulo (\%). La generación de casos inválidos se realiza mediante mutaciones sintácticas, introduciendo errores controlados en cadenas válidas para simular errores comunes.

\subsubsection{Fundamentos teóricos}

Una GLC se define como una tupla G = (V, Σ, P, S), donde:
...
Para expresiones aritméticas, definimos una gramática que incluye precedencia de operadores y paréntesis. La derivación comienza desde S y aplica reglas hasta obtener una cadena terminal.

Los casos extremos se generan aplicando criterios de profundidad máxima del árbol sintáctico o longitud máxima de la cadena. Las mutaciones incluyen eliminación de símbolos, inserción de símbolos inválidos, o cambio de orden.
...
\hline
Derivación & Proceso de aplicar reglas para generar cadenas válidas \\
Mutación sintáctica & Introducción de errores controlados en cadenas válidas \\
Árbol sintáctico & Representación gráfica de la derivación \\
Casos extremos & Cadenas con máxima complejidad o longitud \\
\hline
\end{tabular}
\caption{Conceptos fundamentales del proyecto}
\label{tab:conceptos}
\end{table}
